# Publishing `@koharu/astro-loader` to npm

This process publishes only `packages/astro-loader`. The server, Admin app, and internal UI package remain private
...
The command builds the package, writes `.artifacts/koharu-astro-loader/package.tgz`, enforces a tarball allowlist, scans
...
- the `koharu` npm organization exists and the current account can publish to it;
...
- `npm view @koharu/astro-loader` still returns 404;
- the original artifact downloaded from successful main CI after the Version Packages merge has the same SHA-256
  printed by the `Pack and audit @koharu/astro-loader` step.

Download and inspect the file that will actually be published:

```bash
gh run download <main-ci-run-id> \
  --name koharu-astro-loader-npm-package \
  --dir .artifacts/koharu-astro-loader-ci
shasum -a 256 .artifacts/koharu-astro-loader-ci/package.tgz
tar -xOf .artifacts/koharu-astro-loader-ci/package.tgz package/package.json
```
...
npm publish .artifacts/koharu-astro-loader-ci/package.tgz \
  --access public \
  --tag latest \
  --provenance=false \
...
After the registry smoke passes, mark the early bootstrap package as migrated instead of unpublishing it:

```bash
npm deprecate @coszone/koharu-astro@0.1.0 \
  "Moved to @koharu/astro-loader. Install @koharu/astro-loader instead."
```
...
- Workflow: `stage-astro-loader.yml`
- Environment: `npm`
...
2. manually run `Stage @koharu/astro-loader on npm`, select `latest` or `next`, and confirm staging;
...
Astro sites that never enabled the loader are unaffected. Enabled sites can pin an earlier version or remove the
