# `@koharu/astro-loader` npm 发布

本流程只发布 `packages/astro-loader`。server、Admin 与内部 UI 继续保持 private，不由 npm workflow
发布。
...
1. 构建 package；
2. 生成 `.artifacts/koharu-astro-loader/package.tgz`；
...
registry smoke 通过后，将早期试发包标记为迁移，不执行 unpublish：
...
2. 手动运行 `Stage @koharu/astro-loader on npm`，选择 `latest` 或 `next` 并确认 stage；
...
未启用 loader 的 Astro 站点不受影响，已启用站点可 pin 旧版本或移除 live config 恢复纯静态路径。
