...
- Telegram update stream 以 Bot 为单位而不是以频道为单位。应用层 allowlist 能阻止其他频道
  入库，但 long polling 推进 offset 时仍会确认被忽略的 update；G1.2 不得让同一 Bot 同时运行
  另一个 `getUpdates` consumer。G1.4 必须让一个 collector 统一处理多频道数据库 allowlist。
...
2026-07-24 已在独立测试频道完成真实 smoke：

- 顺序 long polling 归档 1 个 raw update、1 个频道、1 条消息和 1 个 normalized revision；
- `GET /healthz`、`GET /api/v1/channels`、消息列表和详情均返回 `200`；
- 频道发现返回稳定 suite channel ID，列表与详情使用同一稳定 suite message ID；
- 详情包含文本 public projection，且不包含 raw JSON、Telegram update/message/file ID；
- allowlist 外的候选频道没有进入数据库；smoke 完成后停止 collector。
