# G2.6 — 发布 `@coszone/koharu-astro` 设计文档
...
1. 发布 ESM-only `@coszone/koharu-astro`，公开稳定的 typed client、Zod schemas 和 Astro 6 Live Loaders。
...
合并后 Changesets Version Packages PR 把 adapter 提升到 `0.1.0`。首次 publish 是 owner checkpoint：

1. owner 登录 npm、确认属于 `@coszone` organization 且 2FA 可用；
...
5. 后续 workflow 使用 GitHub-hosted runner、`id-token: write` 和 npm CLI `>=11.5.1`，不保存长期 token。
...
- server DTO 与 adapter schema 是两个源码位置；通过真实 Hono response contract test 缓解漂移。
...
5. 从 registry 新建临时 consumer，安装精确 `@coszone/koharu-astro@0.1.0` 再跑一次 smoke。
6. 配置 trusted publisher，后续版本改由受保护 workflow 发布。

回滚：

- 未启用 consumer 无需动作，静态站没有 adapter runtime。
