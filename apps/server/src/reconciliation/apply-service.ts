...
import { reconciliationFindings } from '../db/schema.js';
...
import type { DeterministicRepairService } from './repair.js';
import {
  addReconciliationReportIssue,
  finishReconciliationReport,
  type ReconciliationReport,
} from './report.js';
import { reconciliationScope } from './repository.js';
...
export class ReconciliationApplyService {
  constructor(
    ...
    private readonly persistence: Pick<PostgresReconciliationPersistenceRepository, 'persistScan'>,
    private readonly repair: Pick<DeterministicRepairService, 'apply'>,
  ) {}
  ...
  async apply(input: ReconciliationCliApplyInput): Promise<ReconciliationReport> {
    ...
    const persisted = await this.persistence.persistScan({
    ...
    for (const candidate of candidates.slice(0, RECONCILIATION_APPLY_BATCH_LIMIT)) {
      try {
        const result = await this.repair.apply({
    ...
    return finishReconciliationReport(report, { repaired });
  }
}
