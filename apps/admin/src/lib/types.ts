...
export interface AdminConfigMaskedSecret {
  last4?: string;
  set: boolean;
}
...
export interface AdminConfigSetting {
  description: string;
  effective: AdminConfigMaskedSecret | string | null;
  envName: string;
  kind: 'boolean' | 'number' | 'secret' | 'string';
  label: string;
  locked: boolean;
  pendingRestart: boolean;
  pendingValue?: AdminConfigMaskedSecret | string | null;
  secret: boolean;
  source: 'default' | 'explicit_env' | 'override';
}

export interface AdminConfigSection {
  id: 'ingestion' | 'media_cache' | 'public_api' | 's3';
  label: string;
