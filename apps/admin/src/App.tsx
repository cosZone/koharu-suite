...
function DeskShell({ onSessionRevoked }: { onSessionRevoked(message: string): Promise<void> }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetchJson<AdminConfigResponse>('/api/v1/admin/config', { signal: controller.signal }),
  ...
  async function saveConfig(changes: Record<string, string | null>, reason: string) {
    await runAction('config-save', 'settings', async () => {
      await fetchJson<AdminConfigApplyResult>('/api/v1/admin/config', {
        body: JSON.stringify({ changes, reason }),
        headers: { 'Content-Type': 'application/json' },
        method: 'PUT',
      });
      const nextConfig = await fetchJson<AdminConfigResponse>('/api/v1/admin/config', {
        cache: 'no-store',
      });
      setConfig(nextConfig);
      clearReason('config-save');
      return '已保存，需重启 server 与 worker 后生效。';
    });
  }
  ...
  const desk: Desk = {
    ...
    rawLoading,
    onReasonChange,
    onConfigSave: (changes, reason) => void saveConfig(changes, reason),
