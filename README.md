...
先在 Telegram Desktop 把目标公开频道导出为 JSON。目标频道必须已经由 `kodama channel add`
加入数据库 allowlist；命令不会导入私聊、群组、私有频道或未显式选择的频道。

```bash
# 默认只分析文件和数据库，不写入任何 run、消息或 provenance
pnpm exec kodama import telegram-desktop \
  --input /path/to/result.json \
  --channel=-1001234567890

# 审阅 dry-run 后显式应用；--channel 可重复
pnpm exec kodama import telegram-desktop \
  --input /path/to/result.json \
  --channel=-1001234567890 \
...
退出码 `0` 表示 clean/replay，`2` 表示已完成但存在 conflict 或单条错误，`1` 表示 fatal 或中断。
apply 使用独立 advisory lock、有限批次和可重放 provenance；中断后可用同一文件安全重跑。
Desktop export 只作为 final snapshot：时间明确更新的 snapshot 才能成为 current，stale/ambiguous
内容只进入报告，不覆盖既有归档。G2.1 只保存媒体元数据与受限相对引用，不读取、复制或 hash 媒体
文件，也不会根据 export 中缺失的消息推断删除。

report 只包含计数、频道/消息数字 ID 和有限的脱敏错误，不包含正文、未选择 chat、绝对路径或
secret。apply 只为选中公开频道保存 owner-only source evidence；dry-run 不写数据库。
